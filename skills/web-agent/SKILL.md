...
- **Live events** (the bridge streams console/error/net as they happen): `page.on(kind,fn)` (`'console'|'error'|'net'|'*'`), `page.waitForResponse(url|pred)`, `page.waitForEvent(kind,pred)`, `page.replayEvents({since})` (catch up on the same socket — no backlog/stream race). Set the wait up **before** the action that triggers it. Observe-only (no interception).
...
- **`eval` can't return a Promise** — trigger async work, then read the result in a second `eval` after a beat.
