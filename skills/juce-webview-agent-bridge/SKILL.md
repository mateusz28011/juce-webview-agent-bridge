...
- **Live events** (the bridge streams console/error/net/navigation as they happen): `page.on(kind,fn)` (`'console'|'error'|'net'|'navigation'|'*'`), `page.waitForResponse(url|pred)`, `page.waitForEvent(kind,pred)`, `page.replayEvents({since})` (catch up on the same socket — no backlog/stream race). Set the wait up **before** the action that triggers it. Observe-only (no interception). A `navigation` event (`data:{url,title}`) fires on every page (re)load — use it to detect that a reload wiped your injected state.
